...
log "Test 1/4: mismatch is rejected by default"
...
log "Test 2/4: --force-model-mismatch bypasses the guard"
...
log "Test 3/4: remote embedding works and returns configured model id"
...
if [[ "${WITH_LOCAL_FALLBACK}" -eq 1 ]]; then
  log "Test 4/4: remote failure falls back to local embedding model"
  if [[ -z "${EXTERNAL_REMOTE_URL}" ]]; then
